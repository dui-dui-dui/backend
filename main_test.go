...
func TestLoadConfig(t *testing.T) {
	...
	ioutil.WriteFile("data/config_out.txt", []byte(m["markdown"].(string)), 0644)
}

func TestLoadRegion(t *testing.T) {
	var m map[string]any
	httpGet("http://127.0.0.1:8080/region", &m)
	ioutil.WriteFile("data/region_out.txt", []byte(m["markdown"].(string)), 0644)
}
...
func TestSaveConfig(t *testing.T) {
	...
	md, _ := ioutil.ReadFile("data/config_out.txt")
